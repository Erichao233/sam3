...
#SBATCH --mem=64G
#SBATCH -t 12:00:00
#SBATCH -J sam3_spme_gate_debug_no_newdet
...
# CRITICAL CHANGE: Disable new detections from text prompts
# This forces the tracker to maintain the original ID or fail, preventing "identity takeover" by new detections.
...
OUT_ROOT=${OUT_ROOT:-/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/spme_gate_ovis_debug_pairs_no_newdet_v1/job_${SLURM_JOB_ID:-local}}

# Optional caps for faster debugging (0 = full from init frame)
MAX_FRAMES_PAIRS=${MAX_FRAMES_PAIRS:-0}
TAU=${TAU:-0.5}

# SPME knobs (match your fusion+gate config; override via env)
...
set_gate_env_nodecay() {
  set_gate_env
  export SAM3_SPME_LEARNED_GATE_USE_DECAY=0
}
...
run_suite() {
  ...
  echo "=== [$tag] fusion_only (load ckpt; gate disabled) ==="
  unset_all_spme
  set_common_spme_signal_env
  set_fusion_env
  ...
  echo "=== [$tag] fusion_gate (fusion + gate; log gate stats) ==="
  unset_all_spme
  set_common_spme_signal_env
  set_fusion_env
  set_gate_env
  ...
  echo "=== [$tag] fusion_gate_nodecay (fusion + gate; decay disabled) ==="
  unset_all_spme
  set_common_spme_signal_env
  set_fusion_env
  set_gate_env_nodecay
  python -u scripts/phase0_eval_ovis_signals.py \
    --out-dir "$out_dir/fusion_gate_nodecay" \
    --only-pairs "$pairs" \
    --save-signals 1 \
    --spme-fusion-ckpt "$GATE_CKPT" \
    "${COMMON_ARGS[@]}"

  echo "=== [$tag] gate_only (no fusion injection; log gate stats) ==="
  unset_all_spme
  set_common_spme_signal_env
  export SAM3_SPME_FUSION=1
  ...
  if [ "$ANALYZE" = "1" ]; then
    echo "=== [$tag] analysis ==="
    local ad="$out_dir/analysis"
    mkdir -p "$ad"
    python -u scripts/analyze_ovis_eval.py \
      --baseline-dir "$out_dir/baseline" \
      --run-dir "$out_dir/fusion_only" \
      --run-dir "$out_dir/gate_only" \
      --run-dir "$out_dir/fusion_gate" \
      --run-dir "$out_dir/fusion_gate_nodecay" \
      --metric mean_iou_track_present \
      --subset all \
      --top-k "$TOPK" \
      --bootstrap "$BOOTSTRAP" \
      --seed 0 \
      --write-signal-lists "$ad/pairs_mean_iou_track_present" \
      > "$ad/compare_mean_iou_track_present.txt"
    ...
    python -u scripts/analyze_ovis_eval.py \
      ...
      --bootstrap "$BOOTSTRAP" \
      --seed 0 \
      --write-signal-lists "$ad/pairs_fp_absent_rate_track" \
      > "$ad/compare_fp_absent_rate_track.txt"
  fi
}
