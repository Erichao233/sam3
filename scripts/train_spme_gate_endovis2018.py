...
def main() -> None:
    ...
    ap.add_argument("--gate-frame", type=int, default=14)
    # ap.add_argument("--supervise-frame", type=int, default=2)
    ap.add_argument("--supervise-frame", type=int, default=15)
    ap.add_argument(
        "--randomize-gate-frame",
        action="store_true",
        help="Sample gate/supervise frames per step instead of using fixed --gate-frame/--supervise-frame. "
        "This is a lightweight way to expose the gate to different temporal contexts under long unrolls.",
    )
    ap.add_argument(
        "--gate-frame-min",
        type=int,
        default=1,
        help="Used only when --randomize-gate-frame is set (inclusive).",
    )
    ap.add_argument(
        "--gate-frame-max",
        type=int,
        default=-1,
        help="Used only when --randomize-gate-frame is set (inclusive). -1 => clip_len-2.",
    )
    ap.add_argument(
        "--supervise-gap-min",
        type=int,
        default=1,
        help="Used only when --randomize-gate-frame is set (inclusive). supervise_frame = gate_frame + gap.",
    )
    ap.add_argument(
        "--supervise-gap-max",
        type=int,
    ...
    if not bool(args.randomize_gate_frame):
        ...
        if int(args.gate_frame_min) < 0:
            raise ValueError("--gate-frame-min must be >= 0.")
        if int(args.supervise_gap_min) < 1 or int(args.supervise_gap_max) < 1:
            raise ValueError("--supervise-gap-min/max must be >= 1.")
        if int(args.supervise_gap_min) > int(args.supervise_gap_max):
            raise ValueError("--supervise-gap-min must be <= --supervise-gap-max.")

    _seed_everything(int(args.seed))

    # [ME] Log memory length from environment for scalability tracking
    num_maskmem = os.getenv("SAM3_TRACKER_NUM_MASKMEM", "7")
    print(f"[config] Memory Expansion: SAM3_TRACKER_NUM_MASKMEM={num_maskmem}", flush=True)
    ...
    while True:
        ...
        # Sample gate/supervise frames for this step (optional).
        if bool(args.randomize_gate_frame):
            gate_max = int(args.gate_frame_max)
            if gate_max < 0:
                gate_max = int(args.clip_len) - 2
            gate_min = int(args.gate_frame_min)
            gate_max = min(gate_max, int(args.clip_len) - 2)
            if gate_min > gate_max:
                continue
            gap = rng.randint(int(args.supervise_gap_min), int(args.supervise_gap_max))
            gate_t = rng.randint(gate_min, gate_max)
            sup_t = gate_t + gap
            if sup_t >= int(args.clip_len):
                sup_t = int(args.clip_len) - 1
            if sup_t <= gate_t:
                sup_t = min(int(args.clip_len) - 1, gate_t + 1)
        else:
            gate_t = int(args.gate_frame)
            sup_t = int(args.supervise_frame)

        clip = _choose_clip(
            rng,
            sequences=seqs,
            clip_len=int(args.clip_len),
            supervise_frame=int(sup_t),
