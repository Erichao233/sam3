#!/bin/bash
# EndoVis2017: ablate "Selective Memory Write" (skip memory write on det↔trk mismatch) ON/OFF.
#
# Motivation (paper-safe):
# - Fusion feature injection gives ~0 gain on EndoVis and can regress.
# - A more interpretable and widely used strategy in long-term video segmentation is *memory hygiene*:
#   do not write low-quality / drifted frames into memory, to avoid contamination.
#
...
#
# Tasks:
#   0: Baseline (A), recondition OFF (reference)
#   1: Learned gate only (E), skip-write OFF
#   2: Learned gate only (E), skip-write ON  (Selective Memory Write)
#
# Usage (server):
#   sbatch scripts/sbatch_eval_endovis2017_ablate_skip_write_on_mismatch.sh
#
# Outputs:
#   /home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2017_spme_eval_ablate_skip_write/job_${SLURM_JOB_ID}/task${TASK}_skip${SAM3_SPME_SKIP_WRITE_ON_MISMATCH}/...
...
#SBATCH --mem=64G
#SBATCH -t 12:00:00
#SBATCH -J sam3_endovis2017_eval_ablate_skip_write
...
# ---------------------------------------------------------------------------
# Selective Memory Write (new ablation):
...
export OUT_ROOT="/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2017_spme_eval_ablate_skip_write/job_${SLURM_JOB_ID}/task${TASK}_skip${SAM3_SPME_SKIP_WRITE_ON_MISMATCH}"

echo "[ablate_skip_write] task=$TASK"
echo "[ablate_skip_write] RUN_CONFIGS=$RUN_CONFIGS"
echo "[ablate_skip_write] OUT_ROOT=$OUT_ROOT"
echo "[ablate_skip_write] GATE_CKPT=$GATE_CKPT"
echo "[ablate_skip_write] SAM3_DISABLE_RECONDITION=$SAM3_DISABLE_RECONDITION"
echo "[ablate_skip_write] SAM3_SPME_SKIP_WRITE_ON_MISMATCH=$SAM3_SPME_SKIP_WRITE_ON_MISMATCH"
echo "[ablate_skip_write] SAM3_SPME_SKIP_WRITE_MISMATCH_IOU_THR=$SAM3_SPME_SKIP_WRITE_MISMATCH_IOU_THR"
