#!/bin/bash
# EndoVis2017: ablate a *recovery-safe* skip-write design (hybrid trigger + optional ReID gating).
#
# Motivation:
# - "Uncertainty-only" skip-write (triggered by tracker_score) can block true recovery frames:
#   the tracker may have low score exactly when it re-acquires the object and *needs* to write memory.
# - Pure det↔trk mismatch skip-write is risky under detector domain shift (false mismatches).
#
# Here we test a hybrid trigger:
#   skip_write = (det_trk_iou < thr_iou) AND (tracker_score < thr_trk)
# plus an optional ReID det-vs-trk comparison gate:
#   only skip when detector is *more consistent* with the reference bank than the tracker.
#
...
#   2: F + skip-write hybrid (mismatch AND low tracker_score), det/qcos-gated
#   3: F + skip-write hybrid + ReID det-vs-trk gate (safer)
#
# Usage (server):
#   sbatch scripts/sbatch_eval_endovis2017_ablate_skip_write_hybrid_reid_joint_clip16_mem15.sh
#
# Outputs:
#   /home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2017_spme_eval_ablate_skip_write_hybrid/job_${SLURM_JOB_ID}/task${TASK}/...
...
#SBATCH --mem=64G
#SBATCH -t 12:00:00
#SBATCH -J sam3_endovis2017_eval_ablate_skip_write_hybrid
...
# Reset toggles (enabled per-task).
unset SAM3_SPME_SKIP_WRITE_ON_MISMATCH SAM3_SPME_SKIP_WRITE_EVENT_MODE SAM3_SPME_SKIP_WRITE_TRACKER_THR SAM3_SPME_SKIP_WRITE_MISMATCH_IOU_THR
...
unset SAM3_SPME_REID SAM3_SPME_REID_COMPARE_TRK_DET SAM3_SPME_SKIP_WRITE_REQUIRE_REID_DET_BETTER SAM3_SPME_SKIP_WRITE_REID_MARGIN SAM3_SPME_REID_COMPARE_MARGIN
...
case "$TASK" in
  ...
  2)
    ...
    export SAM3_SPME_SKIP_WRITE_EVENT_MODE="hybrid"
  ...
  3)
    ...
    # ReID safety: only skip-write when detector is *more consistent* than tracker.
    export SAM3_SPME_REID="1"
    export SAM3_SPME_REID_COMPARE_TRK_DET="1"
    export SAM3_SPME_REID_COMPARE_MARGIN="${SAM3_SPME_REID_COMPARE_MARGIN:-0.01}"
    export SAM3_SPME_SKIP_WRITE_REQUIRE_REID_DET_BETTER="1"
    export SAM3_SPME_SKIP_WRITE_REID_MARGIN="${SAM3_SPME_SKIP_WRITE_REID_MARGIN:-$SAM3_SPME_REID_COMPARE_MARGIN}"
    ;;
  *)
...
export OUT_ROOT="/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2017_spme_eval_ablate_skip_write_hybrid/job_${SLURM_JOB_ID}/task${TASK}"

echo "[ablate_skip_write_hybrid] task=$TASK"
echo "[ablate_skip_write_hybrid] RUN_CONFIGS=$RUN_CONFIGS"
echo "[ablate_skip_write_hybrid] OUT_ROOT=$OUT_ROOT"
echo "[ablate_skip_write_hybrid] FUSION_CKPT=$FUSION_CKPT"
echo "[ablate_skip_write_hybrid] GATE_CKPT=$GATE_CKPT"
echo "[ablate_skip_write_hybrid] SAM3_SPME_SKIP_WRITE_EVENT_MODE=${SAM3_SPME_SKIP_WRITE_EVENT_MODE:-none}"
echo "[ablate_skip_write_hybrid] SAM3_SPME_REID=${SAM3_SPME_REID:-0} SAM3_SPME_SKIP_WRITE_REQUIRE_REID_DET_BETTER=${SAM3_SPME_SKIP_WRITE_REQUIRE_REID_DET_BETTER:-0}"
