#!/bin/bash
# EndoVis2018: evaluate joint-trained SPME modules (clip16 + mem15) on official test_data split.
#
# Usage (server):
#   sbatch scripts/sbatch_eval_endovis2018_joint_clip16_mem15.sh
#
# Outputs:
#   /home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2018_spme_eval_joint_clip16_mem15/job_${SLURM_JOB_ID}/...

#SBATCH -A qoscammagpu2
#SBATCH -p pri2021gpu
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH -t 12:00:00
#SBATCH -J sam3_endovis2018_eval_joint_clip16_mem15
#SBATCH -o /home2020/home/icube/kunyuan/SurgBench/SAM/logs/%x-%j.out
#SBATCH -e /home2020/home/icube/kunyuan/SurgBench/SAM/logs/%x-%j.err
#SBATCH --exclude=hpc-n968

set -euo pipefail

# Raw official root + prepared test root.
export ENDOVIS_SRC="${ENDOVIS_SRC:-/home2020/home/icube/kunyuan/SurgBench/surgicaltool/Endovis2018}"
export DATA="${DATA:-/home2020/home/icube/kunyuan/SurgBench/surgicaltool/endovis2018_test}"

# If DATA is not prepared yet, auto-prepare from test_data only.
export PREPARE_DATA="${PREPARE_DATA:-auto}"
export ENDOVIS_GROUPS="${ENDOVIS_GROUPS:-test_data}"

# MICCAI-safe eval defaults.
export OVERLAY_CKPT=""
export PROMPT="visual"
export PROPAGATION_MODE="vg"
export INIT_PROMPT="mask"
export INIT_FRAME_MODE="first_present"
export INIT_SELECT_MODE="prob"
export HOTSTART_DELAY="0"
export DEBUG_SPME_SUMMARY="1"
export SAM3_DISABLE_RECONDITION="1"

# Match the joint model shape/config.
export SAM3_TRACKER_NUM_MASKMEM="${SAM3_TRACKER_NUM_MASKMEM:-15}"
export SAM3_SPME_GATE_HIDDEN="${SAM3_SPME_GATE_HIDDEN:-64}"
export ENDOVIS_FUSION_WEIGHT_MODE="${ENDOVIS_FUSION_WEIGHT_MODE:-eff_iou}"

# ---------------------------------------------------------------------------
# Learned-gate safety envelope (do-no-harm).
# ---------------------------------------------------------------------------
# On EndoVis2018, the learned gate can be brittle under domain shift or under-trained runs.
# Enable the same safe inference-time envelope we use on EndoVis2017:
# - When det is confident + consistent => push (scale→1, offset→0, decay→0)
# - When det is weak/inconsistent     => allow the learned edit.
export SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET="${SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET:-1}"
export SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_USE_QCOS="${SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_USE_QCOS:-1}"
export SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_POW="${SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_POW:-4.0}"

# Keep calibration strengths at identity unless explicitly overridden.
export SAM3_SPME_LEARNED_GATE_OFFSET_STRENGTH="${SAM3_SPME_LEARNED_GATE_OFFSET_STRENGTH:-1.0}"
export SAM3_SPME_LEARNED_GATE_WRITE_STRENGTH="${SAM3_SPME_LEARNED_GATE_WRITE_STRENGTH:-1.0}"
export SAM3_SPME_LEARNED_GATE_DECAY_STRENGTH="${SAM3_SPME_LEARNED_GATE_DECAY_STRENGTH:-1.0}"

# Overseer-style fusion injection (prevents regressions; optional but recommended).
export SAM3_SPME_FUSION_EVENT_DRIVEN="${SAM3_SPME_FUSION_EVENT_DRIVEN:-1}"
export SAM3_SPME_FUSION_TRACKER_THR="${SAM3_SPME_FUSION_TRACKER_THR:-0.8}"

# Joint checkpoint from EndoVis2018 training.
# Use the same checkpoint for fusion and gate so D/E/F all read the same trained spme_* weights.
JOINT_CKPT_DEFAULT="/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2018_spme_joint_train_clip16_mem15/job_16165365/main/checkpoints/spme_gate_latest.pt"
export GATE_CKPT="${GATE_CKPT:-$JOINT_CKPT_DEFAULT}"
export FUSION_CKPT="${FUSION_CKPT:-$GATE_CKPT}"

# Run all major configs for decomposition.
export RUN_CONFIGS="${RUN_CONFIGS:-A D E F}"

export OUT_ROOT="${OUT_ROOT:-/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2018_spme_eval_joint_clip16_mem15/job_${SLURM_JOB_ID}}"

echo "[endovis2018_joint_eval] ENDOVIS_SRC=$ENDOVIS_SRC"
echo "[endovis2018_joint_eval] DATA=$DATA"
echo "[endovis2018_joint_eval] RUN_CONFIGS=$RUN_CONFIGS"
echo "[endovis2018_joint_eval] GATE_CKPT=$GATE_CKPT"
echo "[endovis2018_joint_eval] FUSION_CKPT=$FUSION_CKPT"
echo "[endovis2018_joint_eval] OUT_ROOT=$OUT_ROOT"

bash scripts/sbatch_eval_endovis2018.sh
