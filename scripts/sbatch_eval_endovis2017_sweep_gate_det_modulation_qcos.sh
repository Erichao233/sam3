#!/bin/bash
# EndoVis2017: sweep learned-gate "detector-modulation" with optional qcos consistency.
#
# This is the next step after the det-only modulation sweep:
# - Some ghosts can be driven by high-confidence false detections; det-only modulation can
#   disable the gate exactly when it should intervene.
# - Here we modulate by (det_score * qcos_gate), so the gate is suppressed only when the
#   detector is confident AND consistent with the tracked identity.
#
# Enabled knobs:
# - SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET=1
# - SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_USE_QCOS=1
# - SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_POW swept
#
# Usage (server):
#   sbatch scripts/sbatch_eval_endovis2017_sweep_gate_det_modulation_qcos.sh
#
# Outputs:
#   /home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2017_spme_eval_gate_detmod_qcos/job_${SLURM_JOB_ID}/pow${DET_POW}/...
...
#SBATCH --mem=64G
#SBATCH -t 12:00:00
#SBATCH -J sam3_endovis2017_eval_gate_detmod_qcos
...
export OUT_ROOT="/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/endovis2017_spme_eval_gate_detmod_qcos/job_${SLURM_JOB_ID}/pow${DET_POW}"

echo "[gate_detmod_qcos] task=$TASK DET_POW=$DET_POW"
echo "[gate_detmod_qcos] RUN_CONFIGS=$RUN_CONFIGS"
echo "[gate_detmod_qcos] OUT_ROOT=$OUT_ROOT"
echo "[gate_detmod_qcos] GATE_CKPT=$GATE_CKPT"
echo "[gate_detmod_qcos] FUSION_CKPT=$FUSION_CKPT"
echo "[gate_detmod_qcos] SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET=$SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET"
echo "[gate_detmod_qcos] SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_USE_QCOS=$SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_USE_QCOS"
echo "[gate_detmod_qcos] SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_POW=$SAM3_SPME_LEARNED_GATE_MODULATE_BY_DET_POW"
