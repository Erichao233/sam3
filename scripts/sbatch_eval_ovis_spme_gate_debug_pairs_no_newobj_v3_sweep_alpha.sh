#!/bin/bash
# Sweep inference-time fusion strength for the learned gate checkpoint on the debug-pair sets.
#
# Goal:
# - Keep identity-safe tracking (no new objects from text prompts)
# - Reduce the "always-on fusion" regressions by lowering SAM3_SPME_FUSION_ALPHA(/_OBJ)
# - Output the same comprehensive metrics + a tar.gz you can download for offline analysis.
#
# Usage (server):
#   sbatch scripts/sbatch_eval_ovis_spme_gate_debug_pairs_no_newobj_v3_sweep_alpha.sh
#
# Optional overrides:
#   export GATE_CKPT=/path/to/spme_gate_stepXXXXX.pt
#   export OUT_ROOT=/path/to/outputs/job_${SLURM_JOB_ID}
#   export MAX_FRAMES_PAIRS=0   # 0 = full from init frame
#   export TAU=0.5
#   export SWEEP_ALPHAS="0.05 0.01 0.005 0.002"
#   export SWEEP_ALPHA_OBJS="0.005 0.001 0.0005 0.0002"
#   export FUSION_MODE=film
#   export FUSION_DET_THR=0.0
#   export GATE_OCC_NORM=10.0
#
# Notes:
# - In learned-gate mode, fusion scale is (gate_fusion * base_alpha), so base_alpha is the primary knob.
# - This script intentionally does NOT run locally; it is for SLURM servers.
...
#SBATCH --mem=64G
#SBATCH -t 12:00:00
#SBATCH -J sam3_spme_gate_debug_no_newobj_v3_sweep_alpha
...
OUT_ROOT=${OUT_ROOT:-/home2020/home/icube/kunyuan/SurgBench/SAM/outputs/spme_gate_ovis_debug_pairs_no_newobj_v3_sweep_alpha/job_${SLURM_JOB_ID:-local}}
...
# Default sweep values (space-separated lists).
SWEEP_ALPHAS=${SWEEP_ALPHAS:-"0.05 0.01 0.005 0.002"}
SWEEP_ALPHA_OBJS=${SWEEP_ALPHA_OBJS:-"0.005 0.001 0.0005 0.0002"}
...
set_gate_env() {
  ...
  export SAM3_SPME_LEARNED_GATE_USE_DECAY=1
...
run_fusion_gate_sweep() {
  ...
  alphas=($SWEEP_ALPHAS)
  # shellcheck disable=SC2206
  alpha_objs=($SWEEP_ALPHA_OBJS)

  if [ "${#alphas[@]}" -ne "${#alpha_objs[@]}" ]; then
  ...
  for i in "${!alphas[@]}"; do
    local a="${alphas[$i]}"
    local ao="${alpha_objs[$i]}"
    local a_tag
    a_tag="$(alpha_tag "$a")"

    echo "=== [$tag] fusion_gate_a${a} (alpha_obj=${ao}) ==="
    unset_all_spme
    set_common_spme_signal_env
    set_fusion_env "$a" "$ao"
    set_gate_env
    python -u scripts/phase0_eval_ovis_signals.py \
      --out-dir "$out_dir/fusion_gate_a${a_tag}" \
      --only-pairs "$pairs" \
      --save-signals 1 \
      --spme-fusion-ckpt "$GATE_CKPT" \
      "${COMMON_ARGS[@]}"
...
analyze_metric() {
  ...
  # Build run-dir list: controls + all sweep runs.
  local run_dirs=("$out_dir/fusion_only" "$out_dir/gate_only")
  local -a alphas
  # shellcheck disable=SC2206
  alphas=($SWEEP_ALPHAS)
  for a in "${alphas[@]}"; do
    run_dirs+=("$out_dir/fusion_gate_a$(alpha_tag "$a")")
  done

  local args=(--baseline-dir "$baseline_dir" --metric "$metric" --subset "$subset" --top-k "$TOPK" --bootstrap "$BOOTSTRAP" --seed 0 --write-signal-lists "$out_dir/analysis/pairs_${metric}_${subset}")
  for rd in "${run_dirs[@]}"; do
    args+=(--run-dir "$rd")
  done

  python -u scripts/analyze_ovis_eval.py "${args[@]}" > "$out_dir/analysis/compare_${metric}_${subset}.txt"
}
...
run_suite() {
  ...
  run_baseline_and_controls "$tag" "$pairs"
  run_fusion_gate_sweep "$tag" "$pairs"
...
echo "sweep_alphas=$SWEEP_ALPHAS" >> "$OUT_ROOT/README_run.txt"
echo "sweep_alpha_objs=$SWEEP_ALPHA_OBJS" >> "$OUT_ROOT/README_run.txt"
echo "fusion_mode=$FUSION_MODE det_thr=$FUSION_DET_THR use_presence=$FUSION_USE_PRESENCE" >> "$OUT_ROOT/README_run.txt"
echo "gate_occ_norm=$GATE_OCC_NORM gate_hidden=$GATE_HIDDEN" >> "$OUT_ROOT/README_run.txt"
